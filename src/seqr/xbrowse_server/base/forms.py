...
class EditFamilyForm(forms.Form):
    ...
    coded_phenotype = forms.CharField(required=False)
    about_family_content = forms.CharField(widget=forms.Textarea, required=False)
    analysis_summary_content = forms.CharField(widget=forms.Textarea, required=False)
    ...
    post_discovery_omim_number = forms.CharField(max_length=500, required=False)
    pedigree_image = forms.ImageField(label="Select an image", required=False)


class FAMFileForm(forms.Form):

    fam_file = forms.FileField(max_length=255)
