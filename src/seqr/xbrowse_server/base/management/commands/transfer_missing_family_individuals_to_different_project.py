...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for family, individual_count in list(missing_family_individual_counts.items()):
            missing_individual_counts[individual_count] += 1
            if individual_count > 0:
                updated_families.add(family)

        missing_sample_counts = defaultdict(int)
        for individual, sample_count in list(missing_individual_sample_counts.items()):
        ...
        print("Done.")
        print("----------------------------------------------")
        for num_individuals, num_families in list(missing_individual_counts.items()):
            print('Added {} individuals to {} families'.format(num_individuals, num_families))
        print("----------------------------------------------")
        print('Added {} VCF files'.format(created_vcf_count))
        print("----------------------------------------------")
        for num_samples, num_individuals in list(missing_sample_counts.items()):
