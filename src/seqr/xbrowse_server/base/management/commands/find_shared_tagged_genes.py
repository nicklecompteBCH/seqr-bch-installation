...
class Command(BaseCommand):
    ...
    def print_out(self, gene_to_projects, gene_to_families, gene_to_variants, gene_to_variant_tags, gene_to_variant_and_families):
        ...
        with open(output_filename, 'w') as f:
            ...
            for key, projs in list(sorted(gene_to_projects.items(), key=lambda gene_id_and_projs: len(gene_id_and_projs[1]))):
                ...
                line = "\t".join(map(str, [
                            ...
                            len(variants),
                            ", ".join(variants),
                            ", ".join(["%s (%s)" % (v_and_f[0], ", ".join(v_and_f[1])) for v_and_f in list(gene_to_variant_and_families[key].items())]),
