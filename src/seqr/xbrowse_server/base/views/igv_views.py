...
#    logger.debug("local_settings.!EAD_VIZ_BAM_PATH = %s", settings.!EAD_VIZ_BAM_PATH)
    logger.debug("individual.bam_file_path = %s", individual.bam_file_path)
    complete_bam_path = individual.bam_file_path # os.path.join(settings.!EAD_VIZ_BAM_PATH, individual.bam_file_path)
    if igv_track_name.endswith(".bai"):
        complete_bam_path += ".bai"

    if os.path.isabs(complete_bam_path):
        logger.info("fetch_igv_track() BAM - local file path: %s", complete_bam_path)
        #logger.info("fetch_igv_track() BAM - local file path: %s  (local_settings.!EAD_VIZ_BAM_PATH = %s)", complete_bam_path, settings.!EAD_VIZ_BAM_PATH)
        try:
        ...
        try:
            ...
            if individual.bam_file_path.endswith('.cram'):
                ...
                #proxy_server = settings.!EAD_VIZ_BAM_PATH
                logger.info("fetch_igv_track() BAM - proxy to BAM http server - file path: %s", complete_bam_path) #  (local_settings.!EAD_VIZ_BAM_PATH = %s)", complete_bam_path, settings.!EAD_VIZ_BAM_PATH)
...
def cram_request_proxy(request, path):
    ...
    headers = { key: value for key, value in list(headers.items()) if not key.startswith("X-") and not key.lower() in EXCLUDE_HTTP_HEADERS}
