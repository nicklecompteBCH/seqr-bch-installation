...
def init_command_line_args():
    ...
    p.add_argument("--num-export-partitions", help="Repartition before exporting variants to ES", type=int, default=1000)
    ...
    p.add_argument("--output-vcf", help="(optional) Output vcf filename URL")
...
def compute_output_vds_prefix(args):
    ...
    return 'hdfs:///tmp/{}'.format(hashlib.sha1(args.input_dataset).hexdigest())
...
def step0_init_and_run_vep(hc, vds, args):
    ...
    if args.output_vcf:
        ...
        vds.export_vcf(args.output_vcf)
...
def step3_add_reference_datasets(hc, vds, args):
    ...
    vds = vds.repartition(min(vds.count_variants(), args.num_export_partitions))
...
def cleanup_steps(args):
    ...
    for path in (args.step1_output_vds,  args.step3_output_vds):
        if args.step1_output_vds.startswith("hdfs:///"):
            subprocess.call(['hadoop', 'fs', '-rm', '-f', '-R', '-skipTrash', path])
