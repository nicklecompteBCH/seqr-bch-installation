...
def file_iter(url):
    ...
    if url.startswith('gs://'):
        command = "gsutil cat {}".format(url)
    elif url.startswith('s3://'):
        command = "aws s3 cp {} -".format(url)
    else:
        command = "cat {} ".format(url)
    if url.endswith("gz"):
        command += "| gunzip -c -q -"
    return iter(Popen(command, stdout=PIPE, shell=True).communicate()[0].split('\n'))


def _parse_family_ids_from_fam_file(fam_file_path):
    """Takes a file url and returns a mapping of sample_id to family id"""
    family_ids = set()
    sample_id_to_family_id = collections.OrderedDict()
    for line_counter, line in enumerate(file_iter(fam_file_path)):
