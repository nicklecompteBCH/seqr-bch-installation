...
if __name__ == "__main__":
    ...
    p.add_argument("-vcf","--vcf",help="HDFS path of multisample VCF MatrixTable")
    ...
    p.add_argument("-map","--map_tsv",help="TSV for the indiv id <=> sample id mappings")
    args = p.parse_args()
    ...
    if not args.clinvar:
        ...
        for f in families:
            for s in f.samples:
                fam_dict.update{s.individual_id : s.family_id}
        ...
        def annotate_with_family_id(mt: hl.MatrixTable) -> hl.MatrixTable:
            mt = mt.annotate_cols(
                family_id = hl_fam_dict.get(hl_mapping_dict.get(mt.sample_id))
            )
            return mt
        ...
        filename = 'hdfs:///user/hdfs/out/' + dataset
        finalname = 'hdfs:///user/hdfs/final/' + dataset + ".mt"
        ...
        final : hl.MatrixTable = mt
        #mt = mt.persist(

        print("Preparing for export")
        final.write(finalname)
        export(final,index_name, args.tsv,args.tsves)
        print("Subsetting over families")
        for family in families:
            fam_mt = final.filter_cols(final.family_id == family.family_id)
            fam_mt = annotate_with_genotype_num_alt(fam_mt)
            fam_mt = annotate_with_samples_alt(fam_mt)
            fam_mt = fam_mt.repartition(partition_count)
            fam_mt.write('hdfs:///user/hdfs/final/' + dataset + "_" + family.family_id + ".mt")
            fam_mt = hl.read_matrix_table('hdfs:///user/hdfs/final/' + dataset + "_" + family.family_id + ".mt")
            export(fam_mt,index_name + family.family_id.lower(), args.tsv,args.tsves)
