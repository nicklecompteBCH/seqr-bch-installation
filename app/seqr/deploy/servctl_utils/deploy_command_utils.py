...
def prepare_settings_for_deployment(deployment_target, output_dir, runtime_settings):
    ...
    settings = {key.upper(): value for key, value in list(settings.items())}
...
def delete_pod(component_label, settings, aasync=False, custom_yaml_filename=None):
    deployment_target = settings["DEPLOY_TO"]
    ...
    while is_pod_running(component_label, deployment_target) and not aasync:
        time.sleep(5)
