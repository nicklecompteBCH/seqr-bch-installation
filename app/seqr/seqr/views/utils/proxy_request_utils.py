...
def proxy_request(request, url, host=None, scheme=None, method=None, session=None, headers=None, auth_tuple=None,
    ...
    if filter_request_headers:
        headers = {k: v for k, v in list(headers.items()) if k.lower() not in EXCLUDE_HTTP_REQUEST_HEADERS}
    ...
    if verbose:
        ...
        if headers:
            logger.info("  headers:")
            for key, value in list(sorted(headers.items(), key=lambda i: i[0])):
...
def _convert_django_META_to_http_headers(request_meta_dict):
    ...
    http_headers = {
        ...
        for key, value in list(request_meta_dict.items())
