...
if not opts.dont_add:
    commands += [
        "python3 -u manage.py add_project %(project_id)s",
...
if ped:
    commands += [ "python3 -u manage.py add_individuals_to_project %(project_id)s --ped %(ped)s" ]
...
parallel_commands = ["python3 -u manage.py add_vcf_to_project --load %s %s > logs/%s_weisburd__load_%s.log" % (project_id, vcf, date.strftime(datetime.now(), "%Y_%m_%d"), os.path.basename(vcf))
                     for vcf in opts.vcfs]
