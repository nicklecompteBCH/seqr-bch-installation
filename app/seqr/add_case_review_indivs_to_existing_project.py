...
if xls:
    ped = project_id + ".ped"
    commands += ["python3 -u manage.py convert_xls_to_ped --xls '%(xls)s' --ped '%(ped)s' ",]

commands += [
    "python3 -u manage.py add_individuals_to_project %(project_id)s --ped '%(project_id)s.ped' --case-review ",
