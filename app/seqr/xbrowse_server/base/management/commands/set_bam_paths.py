    ...
    def handle(self, *args, **options):
        """Command line args:
            ...
            arg1 = file path - where file is a tsv table with 2 columns:
                    ...
                    column1: bam file path
                    Before: the path was relative to the base settings.!EAD_VIZ_BAM_PATH url or directory - so that
                    the absolute bam path can be derived by doing os.path.join(settings.!EAD_VIZ_BAM_PATH, column1)
                    Now (Nick's edits): these are absolute paths.
        """
        project_id = args[0]
        ...
        for line in open(args[1]).readlines():
            ...
            absolute_path = bam_path #os.path.join(settings.!EAD_VIZ_BAM_PATH, bam_path)
