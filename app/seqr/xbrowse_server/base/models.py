...
class Family(models.Model):
    ...
    analysis_status_saved_by = models.ForeignKey(User, null=True, blank=True,on_delete=models.PROTECT)
    ...
    def get_analysis_status_json(self):
        return {
            ...
            "date_saved": str(date(self.analysis_status_date_saved)) if self.analysis_status_date_saved is not None else None,
...
class Individual(models.Model):
    ...
    project = models.ForeignKey(Project, null=True, blank=True,on_delete=models.PROTECT)  # move to family only ?
    family = models.ForeignKey(Family, null=True, blank=True,on_delete=models.PROTECT)
    ...
    cram_file_path = models.TextField(default="",blank=True)
...
class FamilySearchFlag(models.Model):
    ...
    def to_dict(self):
        return {
            ...
            'note': self.note,
            'date_saved': str(date(self.date_saved)) if self.date_saved is not None else '',
        }
...
class VariantTag(models.Model):
    project_tag = models.ForeignKey(ProjectTag,on_delete=models.PROTECT)
...
class AnalysedBy(models.Model):
    ...
    def toJSON(self):
        return {
            ...
            'date_saved': str(date(self.date_saved)),
        }
