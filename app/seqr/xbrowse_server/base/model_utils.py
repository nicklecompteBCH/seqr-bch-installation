...
def _convert_xbrowse_kwargs_to_seqr_kwargs(xbrowse_model, include_all=False, **kwargs):
    ...
    if include_all:
        field_mapping = {k: v for k, v in list(field_mapping.items()) if v != _DELETED_FIELD}
    ...
    # handle foreign keys
    for key, value in list(seqr_kwargs.items()):
...
def _create_additional_seqr_entities(xbrowse_model, **kwargs):
    ...
    return {field: entity_func(**seqr_kwargs) for field, entity_func in list(additional_entities_mapping.items())}
