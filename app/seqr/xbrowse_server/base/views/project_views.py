...
def gene_quicklook(request, project_id, gene_id):
    ...
    for project in projects_to_search:
        ...
        for i, variant in enumerate(all_project_variants):
            max_af = max([freq for label, freq in list(variant.annotation['freqs'].items()) if label != "AF"])  # don't filter on within-cohort AF

            if not any([indiv_id for indiv_id, genotype in list(variant.genotypes.items()) if genotype.num_alt > 0]):
                continue
            if max_af >= .01:
                continue
            ...
            if variant_id not in rare_variant_dict:
                ...
                for indiv_id, genotype in list(variant.genotypes.items()):
